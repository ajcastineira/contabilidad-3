...
function Header()
{
    ...
    $this->Cell(150);
    // Título
    $this->Cell(30,20,utf8_decode('PEDIDO Nº: ').utf8_decode($this->datosPresupuesto['NumPedido']),0,0,'R');
    // Salto de línea
    $this->Ln(25);
}
...
function FechasYTipoFactura()
{
    ...
    $this->Cell(50, 4, utf8_decode($this->datosPresupuesto['FechaPedido']),'TR', 0, 'L');
    $this->Cell(10, 4, ' ',0,0, 0);
    // Datos 2º Cuadro: 1 linea
    $this->SetFont('Arial','',9);
    $this->Cell(45, 4, utf8_decode("Tipo Factura: "),'LT', 0, 'R',true);
    $this->Cell(40, 4, utf8_decode($this->datosPresupuesto['TipoFactura']),'TR', 0, 'L');
    $this->Ln();

    // Datos 1º Cuadro: 2 linea
    $this->SetFont('Arial','',9);
    $this->Cell(40, 4, utf8_decode("Fecha Vencimiento: "),'L', 0, 'R',true);
    $this->Cell(50, 4, utf8_decode($this->datosPresupuesto['FechaVtoPedido']),'R', 0, 'L');
    $this->Cell(10, 4, ' ',0,0, 0);
    // Datos 2º Cuadro: 2 linea
    $this->SetFont('Arial','',9);
    $this->Cell(45, 4, utf8_decode("Día: "),'L', 0, 'R',true);
    $this->Cell(40, 4, utf8_decode($this->datosPresupuesto['DiaPeriodica']),'R', 0, 'L');
    $this->Ln();

    // Datos 1º Cuadro: 3 linea
    $this->SetFont('Arial','',9);
    $this->Cell(40, 4, utf8_decode("Fecha Finalización: "),'L', 0, 'R',true);
    $this->Cell(50, 4, utf8_decode($this->datosPresupuesto['FechaFinalizacion']),'R', 0, 'L');
    $this->Cell(10, 4, ' ',0,0, 0);
    // Datos 2º Cuadro: 3 linea
    $this->SetFont('Arial','',9);
    $this->Cell(45, 4, utf8_decode("Frecuencia: "),'L', 0, 'R',true);
    $this->Cell(40, 4, utf8_decode($this->datosPresupuesto['FrecuenciaPeriodica']),'R', 0, 'L');
    $this->Ln();
    
    // Datos 1º Cuadro: 4 linea
    $this->SetFont('Arial','',9);
    $this->Cell(40, 4, utf8_decode("Forma de Pago: "),'L', 0, 'R',true);
    $this->Cell(50, 4, utf8_decode($this->datosPresupuesto['FormaPago']),'R', 0, 'L');
    $this->Cell(10, 4, ' ',0,0, 0);
    // Datos 2º Cuadro: 4 linea
    $this->SetFont('Arial','',9);
    $this->Cell(45, 4, utf8_decode("Fecha Próxima Factura: "),'LB', 0, 'R',true);
    $this->Cell(40, 4, utf8_decode($this->datosPresupuesto['FechaProximaFacturaPeriodica']),'BR', 0, 'L');
    $this->Ln();
    
    // Datos 1º Cuadro: 5 linea
    //si FormaPago =Transferencia presento la cuenta, sino se cierra el cuadro
    if($this->datosPresupuesto['FormaPago'] === 'Transferencia'){
        $this->SetFont('Arial','',9);
        $this->Cell(40, 4, utf8_decode("CC Transferencia: "),'LB', 0, 'R',true);
        $this->Cell(50, 4, utf8_decode($this->datosPresupuesto['CC_Trans']),'BR', 0, 'L');
        $this->Ln();
    }else{
        $this->Cell(90, 4, ' ','T',0, 0);
    }
}
}
...
$pdf = new PDF();
$pdf->setDatosPresupuesto($clsCNContabilidad->datosPedido($_GET['IdPedido']));
...
$fecha=explode('/',$pdf->datosPresupuesto['FechaPedido']);
...
$pdf->Cell(180, 4, utf8_decode($pdf->datosNuestraEmpresa['municipio'].', '.$fecha[0].' de '.$mes.' de '.$fecha[2]),0, 0, 'L');
...
if($_GET['opcion']==='imprimir'){
    ...
    $ejercicio=substr($pdf->datosPresupuesto['NumPedidoBBDD'],0,4);
    $numero=substr($pdf->datosPresupuesto['NumPedidoBBDD'],4,4);
//    $pdf->Output("../pedidosEnviados/Pedido_".$_SESSION['idEmp'].'-'.$ejercicio.'-'.$numero.".pdf","F");
    $pdf->Output("../pedidosEnviados/Pedido_".$_SESSION['idEmp'].'-'.$pdf->datosPresupuesto['NumPedidoBBDD'].".pdf","F");
    $pdf->Close();
}
