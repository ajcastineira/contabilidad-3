...
if(isset($_POST['IdPedido'])){
    ...
    //a pulsado aceptar, si viene POST[IdPedido] = 'Nuevo' es nuevo si llega numero es editar
    if($_POST['IdPedido'] === 'Nuevo' && !isset($varRes)){
        logger('info',basename($_SERVER['PHP_SELF']).'-' ,"Usuario: ".$_SESSION['strUsuario'].', Empresa: '.$_SESSION['strBD'].', SesionID: '.  session_id().
               " ||||Mis Pedidos->Alta|| Ha pulsado 'Aceptar'");
        
        $varRes=$clsCNContabilidad->AltaPedido($_SESSION['usuario'],$_POST);
        
    }else{
        logger('info',basename($_SERVER['PHP_SELF']).'-' ,"Usuario: ".$_SESSION['strUsuario'].', Empresa: '.$_SESSION['strBD'].', SesionID: '.  session_id().
               " ||||Mis Pedidos->Actualizar|| Ha pulsado 'Actualizar'");
        
        $varRes=$clsCNContabilidad->EditarPedido($_SESSION['usuario'],$_POST);
    }
    ...
    if($varRes === false || $varRes === 'false'){
        ...
        if($tieneArticulos === 'on'){
            ...
            $datosPresupuesto = $clsCNContabilidad->datosPedido($varRes);
            ...
            if(is_array($lineasArticulosSinGuardar)){
                ...
                if($_POST['guardarArticulosNuevos'] === 'SI'){
                    //compruebo si este array ($lineasArticulosSinGuardar) tiene datos (es array)
                        ...
                        echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=../vista/articulosSinGuardar.php?peid='.$varRes.'">';die;
        ...
        //pagina de exito
        echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=../vista/exitoInsertado_pedido.php?id='.$varRes.'&pant='.$_POST['pant'].'">';die;
    }
    
}
    
//venimos del menu principal alta o Modificacion/Duplicar/Borrar o de Convertir Presupuesto a Pedido
if(isset($_GET['IdPedido'])){
    logger('info',basename($_SERVER['PHP_SELF']).'-' ,"Usuario: ".$_SESSION['strUsuario'].', Empresa: '.$_SESSION['strBD'].', SesionID: '.  session_id().
           " ||||Mis Pedidos->Modificacion/Duplicar/Baja||");

    $datosPresupuesto=$clsCNContabilidad->datosPedido($_GET['IdPedido']);
    ...
    if(isset($_GET['duplicar']) && $_GET['duplicar']==='si'){
        ...
        $numeroNuevoPedido=$clsCNContabilidad->NumeroNuevoPedido();
        ...
        $numPedido = numeroFormateado($numeroNuevoPedido,$tipoContador);
//        switch ($tipoContador) {
//            case 'simple':
//                $numPedido= date('Y').$numeroNuevoPedido;
//                break;
//            case 'compuesto1':
//                $numeroNuevoPedido=explode('/',$numeroNuevoPedido);
//                while(strlen($numeroNuevoPedido[0])<4){
//                    $numeroNuevoPedido[0]='0'.$numeroNuevoPedido[0];
//                }
//                $numPedido=$numeroNuevoPedido[1].$numeroNuevoPedido[0];
//                break;
//            case 'compuesto2':
//                $numeroNuevoPedido=explode('/',$numeroNuevoPedido);
//                $numPedido=$numeroNuevoPedido[0].$numeroNuevoPedido[1];
//                break;
...
//        }
        ...
        $PedidoDuplicado = $clsCNContabilidad->datosDuplicarPedido($_GET['IdPedido'],$numPedido);
        if($PedidoDuplicado === 'false'){
            echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=../'.$_SESSION['navegacion'].'/error.php?id=No se a podido duplicar el pedido">';die;
        }else{
            echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL=../'.$_SESSION['navegacion'].'/altapedido.php?IdPedido='.$PedidoDuplicado.'">';die;
        }
    }
}else{
    logger('info',basename($_SERVER['PHP_SELF']).'-' ,"Usuario: ".$_SESSION['strUsuario'].', Empresa: '.$_SESSION['strBD'].', SesionID: '.  session_id().
...
$tipoContador=$clsCNContabilidad->Parametro_general('Tipo Contador',date('Y-m-d H:i:s'),date('Y-m-d H:i:s'));
$numPedidoBBDD = numeroFormateado($numeroNuevoPedido,$tipoContador);
...
<TITLE><?php if(isset($_GET['IdPedido'])){echo 'Pedido - EDITAR';}else{echo 'Pedido - ALTA';}?></TITLE>

<SCRIPT type="text/javascript" Language="JavaScript"> 

function validar3()
{
    document.getElementById("cmdFacturar").value = "Enviando...";
    document.getElementById("cmdFacturar").disabled = true;
...
function validar2(esValido,guardarArticulosNuevos)
{
  ...
  textoError='';
    ...
    if(cantidades.length===0){
        textoError=textoError+"Debe introducidir alguna linea en el pedido.\n";
...
//    document.form1.Contacto.style.borderColor='#FF0000';
  ...
  //comprobacion del campo 'numPedido'
  if (document.form1.numPedido.value === ''){ 
    textoError=textoError+"Es necesario introducir un número de pedido.\n";
    document.form1.numPedido.style.borderColor='#FF0000';
    document.form1.numPedido.title ='Se debe introducir un número de pedido';
    esValido.value=false;
  }
  //comprobacion del campo 'FechaPedido'
  if (document.form1.FechaPedido.value === ''){ 
    textoError=textoError+"Es necesario introducir una fecha de pedido.\n";
    document.form1.FechaPedido.style.borderColor='#FF0000';
    document.form1.FechaPedido.title ='Se debe introducir una fecha de pedido';
    esValido.value=false;
  }
...
function borrarPedido(id){
    if (confirm("¿Desea borrar el pedido de la base de datos?"))
    {
        window.location='../vista/pedidoBorrar.php?id='+id;
    }
}
...
function Imprimir(i){
    if(document.form1.SePuedeImprimir.value==='SI'){    
        ...
        window.open("../vista/pedidoImprimir.php?"+datos, '', 'scrollbars=yes,menubar=no,height=600,width=800,resizable=yes,toolbar=no,status=no,location=no');
    }else{
        alert('No se puede ver el PDF sin haber guardado el pedido');
    }
}
...
function Enviar(i){
    if(document.form1.SePuedeImprimir.value==='SI'){    
        ...
        $.ajax({
          url: '../vista/pedidoImprimir.php?'+datos,
          type:"get"
        });
    }else{
        alert('No se puede enviar el PDF sin haber guardado el pedido');
    }
}
...
function preparaURL(i,opcion){
        var datos='IdPedido=<?php echo $_GET['IdPedido']; ?>';
...
function ComprobarPedido(objeto){
    var IdPedido='<?php if(isset($_GET['IdPedido'])){echo $_GET['IdPedido'];}else{echo 'NO';}?>';
    $.ajax({
      data:{"numP":objeto.value,"IdPedido":IdPedido},  
      url: '../vista/ajax/ComprobarNumeroPedido.php',
      type:"get",
      success: function(data) {
        if(data==='SI'){
            alert('Este número de pedido EXISTE.');
            objeto.focus();
        }
      }
    });
...
<script type="text/javascript" Language="JavaScript"> 
//function que renderiza la linea del pedido    
function lineaFactura(linea,idPedidoLinea,numLineaPedido,cantidad,concepto,precio,importe,iva,cuota,total,IdArticulo){
...
?>
    $("#linea<?php echo $i;?>:last").after(
            lineaFactura(<?php echo $i+1;?>,<?php echo $idPedidoLinea; ?>,<?php echo $numLineaPedido; ?>,'<?php echo $cantidadPres; ?>','<?php echo str_replace(array("\r\n","\r","\n"),'\n',$conceptoPres); ?>','<?php echo $precioPres; ?>','<?php echo $importePres; ?>',
...
?>
  ...
  //habilita el campo 'CC_Trans' si el campo '' = Transferencia
  SiEsTransferencia(document.form1.FormaPagoHabitual);
});
...
<script type="text/javascript" Language="JavaScript">     
function focus(){
    document.form1.numPedido.focus();
}
</script>
</head>
...
//              SiEsContado(document.form1.FormaPagoHabitual);
              <?php
                if(isset($_GET['borrar'])&& $_GET['borrar']=='si'){
                    echo 'borrarPedido('. $_GET['IdPedido'].');';
                }
                if(isset($datosPresupuesto)){
...
<!--    cabecera-->
<?php
if(isset($_GET['IdPedido']) || isset($varRes)){
    $fechaForm=$datosPresupuesto['FechaPedido'];
}else{
...
<form id="form1" name="form1" method="post" action="../vista/altapedido.php" onSubmit="desactivaBoton();">
    ...
    <tr> 
        ...
        <td>
            ...
            <table border="0" class="cabecera">
                ...
                <tr align="right">
                    <td></td>
                    <td height="25" align="right">Pedido Nº:</td>
                    <td>
                        <input type="text" class="textbox1" name="numPedido" size="12" tabindex="1" <?php if($tipoContador<>''){echo ' readonly';}?>
                               value="<?php if(isset($datosPresupuesto['NumPedido'])){echo $datosPresupuesto['NumPedido'];}else{echo $numeroNuevoPedido;}?>"
                               onkeypress="DesactivaImprimir();" onkeyup="DesactivaImprimir();"
                               onMouseOver="onMouseOverInputText(this);" onMouseOut="onMouseOutInputText(this);"
                               onfocus="onFocusInputText(this);" onblur="onBlurInputText(this);ComprobarPedido(this);" />
                        <input type="hidden" name="numPresupuestoBBDD" value="<?php if(isset($datosPresupuesto['NumPedidoBBDD']) && $datosPresupuesto['NumPedidoBBDD'] !== ''){echo $datosPresupuesto['NumPedidoBBDD'];}else{echo $numPedidoBBDD;} ?>" />
...
<table border="0" width="<?php echo $anchoCabecera; ?>">
    <tr>
        <td width="50%" height="75" align="left" valign="bottom">
            ...
            <input type="hidden" name="fechaPedido" value="<?php echo $fechaTexto;?>"/>
        </td>
        <td width="50%" style="background-color: #eeeeee;">
            <table border="0" width="<?php echo $anchoCabecera/2; ?>">
                ...
                <tr>
                    ...
                    <td align="left">
                        <select class="textbox1" name="Contacto" id="Contacto" style="width: 100%;" tabindex="2" onchange="rellenarDatos(this.value,'Nuevo');DesactivaImprimir();" onblur="SiEsTransferencia(document.form1.FormaPagoHabitual);">
                            <?php echo $htmlSelect; ?>
                        </select>
...
<!--    fin cabecera-->
  ...
  <div class="docPresup"> 
    ...
    <?php
    include_once '../vista/pedido_cabecera.php';
    ?>
    ...
    <input type="hidden" name="esValido" value="false"/>     
      ...
      <?php
        if(isset($_GET['IdPedido'])){
            echo '<input type="hidden"  name="IdPedido" value="'.$_GET['IdPedido'].'" />';
        }else if(isset($varRes)){
            echo '<input type="hidden"  name="IdPedido" value="'.$varRes.'" />';
        }else{
            echo '<input type="hidden"  name="IdPedido" value="Nuevo" />';
        }
      ?>  
      <P>
        <input type="hidden" name="guardarArticulosNuevos" value="NO"/>
        <input type="button" id="cmdAlta" name="cmdAlta" class="button" value = "<?php if(isset($_GET['IdPedido'])){echo 'Grabar';}else{echo 'Grabar';} ?>" onclick="javascript:validar2(document.form1.esValido,document.form1.guardarArticulosNuevos);" />
        ...
        if(isset($_GET['IdPedido'])){
            echo '<input type="button" class="buttonAzul" value="Eliminar" name="cmdBorrar" onclick="javascript:borrarPedido('.$_GET['IdPedido'].');" />';
        } 
        if(isset($varRes)){
            echo '<input type="button" class="buttonAzul" value="Eliminar" name="cmdBorrar" onclick="javascript:borrarPedido('.$varRes.');" />';
        } 
        ?>  
        ...
        <?php if(!isset($_GET['IdPedido']) && !isset($varRes)){echo '<input type="hidden" name="SePuedeImprimir" value="NO" />';}else{echo '<input type="hidden" name="SePuedeImprimir" value="SI" />';}?>
      </P>
...
<div id="inline">
	<h2>Envio PDF</h2>
        ...
        <form id="contact" name="contact" action="../vista/pedidoEnviar.php" method="post">
