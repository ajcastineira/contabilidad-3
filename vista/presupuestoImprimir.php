...
function Header()
{
    ...
    $this->Cell(150);
    // Título
    $this->Cell(30,20,utf8_decode($this->datosPresupuesto['Titulo'].' nº: ').utf8_decode($this->datosPresupuesto['NumPresupuesto']),0,0,'R');
    // Salto de línea
    $this->Ln(25);
}
...
function Footer()
{
    ...
    $numPalabras=explode(' ',utf8_decode($this->datosPresupuesto['Texto Pie']));
...
function DatosNuestrosYCliente()
{
    ...
    $this->Cell(55, 4, $this->datosNuestraEmpresa['provincia'],0,'L', 'L');
    $this->Cell(30, 4, ' ',0,0, 0);
    ...
    $this->Cell(55, 4, 'CIF: '.$this->datosNuestraEmpresa['strCIF'],0,'L', 'L');
    $this->Cell(30, 4, ' ',0,0, 0);
...
$pdf = new PDF();
$datosPresupuesto=$clsCNContabilidad->datosPresupuesto($_GET['IdPresupuesto']);
$datosPresupuesto['Texto Pie']=$clsCNContabilidad->Parametro_general('Texto Pie',date('Y-m-d H:i:s'),date('Y-m-d H:i:s'));
$datosPresupuesto['Titulo Presupuesto']=$clsCNContabilidad->Parametro_general('Titulo Presupuesto',date('Y-m-d H:i:s'),date('Y-m-d H:i:s'));

$datosPresupuesto['Titulo']='PRESUPUESTO ';
if($datosPresupuesto['Titulo Presupuesto']==='oferta'){
    $datosPresupuesto['Titulo']='OFERTA ';
}
if($datosPresupuesto['Proforma']==='1'){
    $datosPresupuesto['Titulo']='OFERTA PROFORMA ';
}


$pdf->setDatosPresupuesto($datosPresupuesto);
...
$pdf->Ln();
$pdf->Ln();
$fecha=explode('/',$pdf->datosPresupuesto['FechaPresupuesto']);
...
//aqui conpruebo si todos los valores de Cantidad y Precio son 0, entonces estas dos columnas nolas pongo en
//la factura
$cantidadEsCero='SI';
$precioEsCero='SI';

for($i=0;$i<count($pdf->datosPresupuesto['DetallePresupuesto']);$i++){
    if($pdf->datosPresupuesto['DetallePresupuesto'][$i]['cantidad']<>'0'){
        $cantidadEsCero='NO';
        break;
    }
...
if($CONcolumnas==='SI'){// van las columnas de cantidad y precio
    ...
    for ($i=0;$i<count($pdf->datosPresupuesto['DetallePresupuesto']);$i++){
        ...
        for($j=0;$j<count($lineas);$j++){
            ...
            if($j==0){
                if($pdf->datosPresupuesto['DetallePresupuesto'][$i]['cantidad']==='0'){
                    $pdf->datosPresupuesto['DetallePresupuesto'][$i]['cantidad']='';
                }
                $pdf->Cell($columCantidad+0.1, $altura, formateaNumeroContabilidad($pdf->datosPresupuesto['DetallePresupuesto'][$i]['cantidad']),'L',0,'R',$pdf->fill);
            }else{
            ...
            if($j==0){
                if($pdf->datosPresupuesto['DetallePresupuesto'][$i]['precio']==='0'){
                    $pdf->datosPresupuesto['DetallePresupuesto'][$i]['precio']='';
                }
            ...
            if($j==0){
                $pdf->Cell($columImporte, $altura, formateaNumeroContabilidad($pdf->datosPresupuesto['DetallePresupuesto'][$i]['importe']),'L',0,'R',$pdf->fill);
            }else{
                $pdf->Cell($columImporte, $altura, '','L',0,'R',$pdf->fill);
            }
            if($j==0){
                $pdf->Cell($columIva, $altura, $pdf->datosPresupuesto['DetallePresupuesto'][$i]['iva'],'L',0,'R',$pdf->fill);
            }else{
                $pdf->Cell($columIva, $altura,'' ,'L',0,'R',$pdf->fill);
            }
            if($j==0){
                $pdf->Cell($columCuota, $altura, formateaNumeroContabilidad($pdf->datosPresupuesto['DetallePresupuesto'][$i]['cuota']),'L',0,'R',$pdf->fill);
            }else{
                $pdf->Cell($columCuota, $altura,'' ,'L',0,'R',$pdf->fill);
            }
            if($j==0){  
                $pdf->Cell($columTotal-0.2, $altura, formateaNumeroContabilidad($pdf->datosPresupuesto['DetallePresupuesto'][$i]['total']),'L',0,'R',$pdf->fill);
                $pdf->SetLineWidth(0.1);
                $pdf->Cell(0.1, 6, '','R',0,'R');
            }else{
        ...
        $totalImporte=$totalImporte+$pdf->datosPresupuesto['DetallePresupuesto'][$i]['importe'];
        $totalCuota=$totalCuota+$pdf->datosPresupuesto['DetallePresupuesto'][$i]['cuota'];
    }
    //linea inferior
...
$pdf->Cell($columTotal, $altura, formateaNumeroContabilidad($totalImporte+$totalCuota),'TR','LT', 'R',true);
$pdf->Ln();
...
$pdf->Cell(27, $altura, utf8_decode($pdf->datosPresupuesto['FormaPago']),0,'R', 'L',true);
$pdf->Cell(48, $altura, utf8_decode('Condiciones válidas durante '),0,'L', 'R');
$pdf->Cell(10, $altura, $pdf->datosPresupuesto['Validez'],0,'R', 'C',true);
$pdf->Cell(10, $altura, utf8_decode('días desde la fecha de emisión de la propuesta'),0,'L', 'L');
$pdf->Ln();
...
if($_GET['opcion']==='imprimir'){
    ...
    $ejercicio=substr($pdf->datosPresupuesto['NumPresupuestoBBDD'],0,4);
    $numero=substr($pdf->datosPresupuesto['NumPresupuestoBBDD'],4,4);
//    $pdf->Output("../presupuestosEnviados/Presupuesto_".$_SESSION['idEmp'].'-'.$ejercicio.'-'.$numero.".pdf","F");
    $pdf->Output("../presupuestosEnviados/Presupuesto_".$_SESSION['idEmp'].'-'.$pdf->datosPresupuesto['NumPresupuestoBBDD'].".pdf","F");
    $pdf->Close();
}
?>
