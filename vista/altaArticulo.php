...
if(isset($_POST['Id'])){
    ...
    if($_POST['Id'] === ''){
        ...
        $varRes=$clsCNContabilidad->AltaArticulo($_POST);
        ...
        $varRes=$clsCNContabilidad->EditarArticulo($_POST);
    ...
    if(isset($_GET['Id']) && $_GET['Id'] !== ""){
        $datos = $clsCNContabilidad->LeeArticulo($_GET['Id']);
    }
    
    //compruebo si traigo un grupo por post
    $grupo = '0';//por defecto 0-SIN GRUPO
    ...
    if($_SESSION['navegacion']==='movil'){
        html_paginaArticuloMovil($datos,$clsCNContabilidad,$grupo);
    }else{
        html_paginaArticulo($datos,$clsCNContabilidad,$grupo);
    }
    
}

function html_paginaArticulo($datos,$clsCNContabilidad,$grupo){
?>
<!DOCTYPE html>
<HTML>
<HEAD>
...
<TITLE><?php if(isset($_GET['Id'])){echo 'Artículo - EDITAR';}else{echo 'Artículo - ALTA';}?></TITLE>

<script language="JavaScript">

function validar2(Id)
{
    //si viene un Id es editar, sino es nuevo
    ...
    $.ajax({
      data:{"opcion":"Referencia","dato":document.form1.Referencia.value},  
      url: '../vista/ajax/articuloComprobarExiste.php',
      type:"get",
      success: function(data) {
        if(data === 'SI' && Id === ''){//es nuevo
            textoError=textoError+"La Referencia de artículo introducida ya existe en la base de datos.\n";
            document.form1.Referencia.style.borderColor='#FF0000';
            document.form1.Referencia.title ='La Referencia del artículo introducida ya existe en la base de datos';
            esValido=false;
        }
        ...
        $.ajax({
          data:{"opcion":"Descripcion","dato":document.form1.Descripcion.value},  
          url: '../vista/ajax/articuloComprobarExiste.php',
          type:"get",
          success: function(data) {
            if(data === 'SI' && Id === ''){//es nuevo
                ...
                document.form1.Descripcion.title ='La Descripcion del artículo introducida ya existe en la base de datos';
                esValido=false;
            }
            ...
            if (document.form1.Referencia.value == ''){ 
              ...
              document.form1.Referencia.style.borderColor='#FF0000';
              document.form1.Referencia.title ='Se debe introducir la Referencia';
              esValido=false;
            }
            ...
            if (document.form1.Descripcion.value == ''){ 
              ...
              document.form1.Descripcion.style.borderColor='#FF0000';
              document.form1.Descripcion.title ='Se debe introducir la descripción';
              esValido=false;
            }
            //comprobacion del campo 'Precio'
            if (document.form1.Precio.value == ''){ 
              textoError=textoError+"Es necesario introducir El precio.\n";
              document.form1.Precio.style.borderColor='#FF0000';
              document.form1.Precio.title ='Se deben introducir El precio';
              esValido=false;
            }
            ...
            if (document.form1.cantidadAlmacen.value == ''){ 
              ...
              document.form1.cantidadAlmacen.style.borderColor='#FF0000';
              document.form1.cantidadAlmacen.title ='Se debe introducir la cantidad del almacén';
              esValido=false;
            }
...
if(isset($_GET['Id'])){
    ...
    $tituloForm='EDITAR ARTÍCULO';
}else{
    $cabeceraNumero='010701';
    $tituloForm='ALTA DE ARTÍCULO';
}
...
?>
  <div class="doc"> 
    <hr color = "#FF9900">
    <br/>
    <form name="form1" method="post" action="../vista/altaArticulo.php" onSubmit="desactivaBoton();">
        ...
        <tr> 
          ...
          <td>
              ...
              <input class="textbox1" type="text" name="Descripcion" id="descripcion" maxlength="255"
                     value="<?php if(isset($datos[0]['Descripcion'])){echo $datos[0]['Descripcion'];}?>"
                     onMouseOver="onMouseOverInputText(this);" onMouseOut="onMouseOutInputText(this);" 
                     onfocus="onFocusInputText(this);" onblur="onBlurInputText(this);" />
              </div>
          </td>
        ...
        <tr> 
          ...
          <td>
              ...
              <select name="tipoIVA"
                  ...
                  $selected10='';
                  $selected21='';
                  if(isset($datos[0]['tipoIVA'])){ //existe IVA
                      if($datos[0]['tipoIVA']==0){
                          $selected0='selected';
                      }else if($datos[0]['tipoIVA']==4){
                          $selected4='selected';
                      }else if($datos[0]['tipoIVA']==10){
                          $selected10='selected';
                      }else if($datos[0]['tipoIVA']==21){
                          $selected21='selected';
                      }
          ...
          <td>
              <div align="left">
                ...
                for ($i = 0; $i < count($listadoCuentasContables); $i++) {
                    $selected='';
                    if($datos[0]['CuentaContable']===$listadoCuentasContables[$i]['cuenta']){
                        $selected='selected';
                    }
          ...
          <td colspan="2">
              <div align="left">
                ...
                for ($i = 0; $i < count($arResult); $i++) {
                    $selected='';
                    if($datos[0]['IdGrupo']===$arResult[$i]['IdGrupo']){
                        $selected='selected';
                    }
                    echo"<option value='".$arResult[$i]['IdGrupo']."' ".$selected.">".$arResult[$i]['Grupo']."</option>";
                }
                ?>
                </select>
              </div>
      ...
      <P>
        ...
        if(isset($_GET['Id'])){
        ?>
            <input type="button" class="buttonAzul" name="cmdBorrar" value = "Borrar" onclick="javascript:borrarArticulo('<?php echo $_GET['Id']; ?>');" /> 
        <?php
        }
        ?>
        <input type="button" class="button" name="cmdAlta" id="cmdAlta" value = "<?php if(isset($datos[0]['Id'])){echo 'OK';}else{echo 'Proceder al Alta';}?>" onclick="javascript:validar2('<?php echo $_GET['Id']; ?>');" /> 
...
function html_paginaArticuloMovil($datos,$clsCNContabilidad,$grupo){
?>
<!DOCTYPE html>
<html>
<head>
